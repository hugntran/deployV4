...
const adminNavItems: NavItem[] = [
  ...
  { name: "🅿️ Parking & Charging station", path: "/manage-parking-charging" },
  { name: "🚗 Lot", path: "/slot-list" },
  { name: "🎁 Voucher", path: "/voucher-management-list" },
  { name: "🎟️ Ticket", path: "/ticket-management" },
  { name: "🧾Invoice", path: "/invoice-list" },
  { name: "💵 Refund", path: "/refund-list" },
  { name: "👤 Customer", path: "/customer-management" },
  { name: "🧑‍💼 Staff", path: "/not-customer-management" },
  { name: "📋 Complaint", path: "/admin-complaint-page" },
];

const staffNavItems: NavItem[] = [
  { name: "📊 Dashboard", path: "/home" },
  { name: "🚗 Lot", path: "/staff-slot-list" },
  { name: "🎟️ Ticket", path: "/staff-ticket-management" },
  { name: "📋 Complaint", path: "/admin-complaint-page" },
];
...
const AppSidebar: React.FC = () => {
  ...
  useEffect(() => {
    const fetchComplaintCount = async () => {
      try {
        ...
        setComplaintCount(pendingCount);
      } catch (err) {
  ...
  const selectedNavItems = userRole === "ADMIN" ? adminNavItems : staffNavItems;

  const computedNavItems: NavItem[] = selectedNavItems.map((item) => {
    if (item.name.includes("Complaint")) {
      return {
        ...item,
        name: complaintCount > 0 ? "📋 Complaint (!)" : "📋 Complaint",
      };
    }
    return item;
  });
      ...
      {items.map((nav, index) => (
        <li key={nav.name}>
          {nav.subItems ? (
            ...
            >
              <span className={`menu-item-icon-size  ${openSubmenu?.type === menuType && openSubmenu?.index === index ? "menu-item-icon-active" : "menu-item-icon-inactive"}`}></span>
            ...
            nav.path && (
              <Link to={nav.path} className={`menu-item group ${isActive(nav.path) ? "menu-item-active" : "menu-item-inactive"}`}>
                <span className={`menu-item-icon-size ${isActive(nav.path) ? "menu-item-icon-active" : "menu-item-icon-inactive"}`}></span>
                {(isExpanded || isHovered || isMobileOpen) && <span className="menu-item-text">{nav.name}</span>}
              </Link>
            )
          )}
  ...
  return (
    ...
    >
      <div className={`py-8 flex items-center gap-2 ${!isExpanded && !isHovered ? "lg:justify-center" : "justify-start"}`}>
        <Link to="/home" className="flex items-center gap-2">
          {isExpanded || isHovered || isMobileOpen ? (
            <>
              <img src="./images/logo.png" alt="Logo" width={50} />
              <span className="font-semibold text-lg select-none">Pogo Admin</span>
            </>
          ) : (
