...
const AppHeader: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAllLocations = async () => {
      ...
      try {
        while (hasNextPage) {
          const res = await fetch(`${API_BASE_URL}/app-data-service/locations/nearby?longitude=${longitude}&latitude=${latitude}&maxDistance=${maxDistance}&page=${page}&size=${size}`, {
            headers: {
      ...
      <div className="flex flex-col items-center justify-between grow lg:flex-row lg:px-6">
        <div className="flex items-center justify-between w-full gap-2 px-3 py-3 border-b border-gray-200 dark:border-gray-800 sm:gap-4 lg:justify-normal lg:border-b-0 lg:px-0 lg:py-4">
          ...
          <Link to="/home" className="lg:hidden">
            <img src="./images/logo.png" alt="Logo" width={40} height={12} />
          </Link>
          {userRole === "STAFF" && <p>📍{locationName}</p>}
          <button
